...
import org.eclipse.paho.android.sample.service.MessageStore.StoredMessage;
...
class MqttConnection implements MqttCallback {
	...
	public void connect(MqttConnectOptions options, String invocationContext, String activityToken) {
		EnsLogUtil.logD(TAG, "ENTER connect>>>>");
		...
		EnsLogUtil.logD(TAG, "ENTER connect>>>> Connecting {" + serverURI + "} as {"+ clientId + "}");
			...
			if (persistence == null) {
				...
				if (myDir == null) {
					...
					if(myDir == null){
						//Shouldn't happen.
						resultBundle.putString(
								MqttServiceConstants.CALLBACK_ERROR_MESSAGE, "Error! No external and internal storage available");
		...
		EnsLogUtil.logD(TAG, "EXIT connect>>>>");
	}
